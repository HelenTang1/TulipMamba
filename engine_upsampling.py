...
# TODO: MC Drop
@torch.no_grad()
def MCdrop(data_loader, model, device, log_writer, args=None, image_cols = 1024):
    '''Evaluation without Monte Carlo Dropout'''
    print("MCdrop")
    ...
    for batch in tqdm.tqdm(data_loader):
        ...
        if log_writer is not None:
            ...
            if args.dataset_select == "carla":
                ...
                # 3D Evaluation Metrics
                pcd_pred = img_to_pcd_kitti(pred_img, maximum_range= 80, image_cols = image_cols)
                pcd_gt = img_to_pcd_kitti(images_high_res, maximum_range = 80, image_cols = image_cols)
