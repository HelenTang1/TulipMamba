...
class PatchEmbedding(nn.Module):
    def __init__(self, img_size=(16, 1024), 
        ...
        # stride = path_size -> each patch embeded by itself
        # if circular padding
        if circular_padding:
        ...
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity() # default norm_layer=nn.LayerNorm
        self.grid_size = (img_size[0] // patch_size[0], img_size[1] // patch_size[1])
        self.num_patches = self.grid_size[0] * self.grid_size[1]

    # This should pad to make sure the W, H, are divisible by patch size
    # But I'm not sure why it mix self.patch_size[0] and self.patch_size[1]
    ...
    # because kernel_size = 8, and stride = 4, so padding = 2(*2) to retain the grid size
    def circularpadding(self, x: torch.Tensor) -> torch.Tensor:
        x = func.pad(x, (2, 2, 0, 0), "circular")
        return x

    def forward(self, x):
        x = self.padding(x)  # (B, C, grid_H, grid_W)
        ...
        # it computes mean/var across the C=embed_dim features only 
        # (not across B, H, or W)
        x = self.norm(x)
        return x # (B, grid_H, grid_W, C)
...
class PatchMerging(nn.Module):
    def __init__(self, dim: int, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.norm = norm_layer(4 * dim) # after merging the dim will *4
        self.reduction = nn.Linear(4 * dim, 2 * dim, bias=False) # map dim to * 2
...
class Mlp(nn.Module):
    def __init__(self, in_features: int, hidden_features: int = None, out_features: int = None,
        ...
        super().__init__()
        out_features = out_features or in_features # default = in_features
...
class SwinTransformerBlock(nn.Module):
    def __init__(self, 
        ...
        self.mlp = Mlp(in_features=dim, 
                       hidden_features=mlp_hidden_dim, # input * 4
    ...
    def forward(self, x):
        ...
        x = self.attn(x)
        # residual with drop
        # as for each x(whole feature map) in batch
        # if not drop: x = (x/keep_prob)
        # if drop: x = 0
        # Why (x/keep_prob)? -> keep the E[x]=x same as without drop
        x = self.drop_path(x)
        # add residial (may dropped during train, no dropped during eval)
        x = x + x_copy
...
class BasicBlock(nn.Module):
    def __init__(self, index: int, 
        ...
        depth = depths[index]
        dim = embed_dim * 2 ** index # 96* (2^index) -> (96, 192, 384, 768)
        num_head = num_heads[index]
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                mlp_ratio=mlp_ratio,# hidden dim multiply factor, but out_features = in_features
                qkv_bias=qkv_bias,
                drop=drop_rate, # 0
                attn_drop=attn_drop_rate, # 0
                drop_path=drop_path_rate[i],# drop rate linear ascending with depth
                norm_layer=norm_layer)
            for i in range(depth)])

        if patch_merging:
            self.downsample = PatchMerging(dim=embed_dim * 2 ** index, 
                                           norm_layer=norm_layer)
        else:
            self.downsample = None
...
# change the defaults to the KITTI dataset, for easier learning
class TULIP(nn.Module):
    ...
    def build_layers(self):
        ...
        for i in range(self.num_layers):
            layer = BasicBlock(
                ...
                drop_path=self.drop_path, #drop_path_rate = 0.1
                window_size=self.window_size, #(2, 8)
    ...
    def forward(self, x, target, eval = False, mc_drop = False):
        ...
        x = self.patch_embed(x)  # (B, grid_H, grid_W, C)
        x = self.pos_drop(x) #drop rate = 0
        x_save = []
        for i, layer in enumerate(self.layers):
            # save the first~(bottom-1)
            # bottom will not save
            # acutally save the feature after patch merging
            x_save.append(x) 
            x = layer(x)
            
        x = self.first_patch_expanding(x) # path unmerging


        for i, layer in enumerate(self.layers_up):
            # concat -> project -> layer_up
            x = torch.cat([x, x_save[len(x_save) - i - 2]], -1) # bottom -1 = x_save[len(x_save) - 1]
...
# ADD: more options
#----------------------------NEW------------------------
# These are all based on tulip_base
from .tulip_bidir_addfusion import tulip_base_mamba_crsatten, tulip_base_mamba_crsmamba

from .tulip_newest_two import tulip_base_mamba
