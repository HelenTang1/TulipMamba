...
def read_args():
    ...
    parser.add_argument('--image_rows', type=int, default=64)
    parser.add_argument('--image_cols', type=int, default=1024)
    parser.add_argument('--num_data_train', type=int, default=21000)
    parser.add_argument('--num_data_val', type=int, default=2500)
    parser.add_argument("--input_path", type=str , default="./dataset/kitti")
    parser.add_argument("--output_path", type=str , default="./dataset/kitti")
    ...
    parser.add_argument("--train_split_path", type = str, default = "./TULIP/kitti_utils/train_files.txt")
    parser.add_argument("--val_split_path", type = str, default = "./TULIP/kitti_utils/val_files.txt")
...
def main(args):
    ...
    output_dir_name_train = os.path.join(args.output_path, args.output_path_name_train)
    pathlib.Path(output_dir_name_train).mkdir(parents=True, exist_ok=True)
    if args.create_val:
        output_dir_name_val = os.path.join(args.output_path, args.output_path_name_val)
    ...
    if num_data_train < len(train_split):
        ...
        for train_folder in train_split:
            if not os.path.exists(os.path.join(dir_name, train_folder)):
                print("Not exist:", os.path.join(dir_name, train_folder))
                continue
    ...
    if args.create_val:
        if num_data_val < len(val_split):
            ...
            for val_folder in val_split:
                if not os.path.exists(os.path.join(dir_name, val_folder)):
                    print("Not exist:", os.path.join(dir_name, val_folder))
                    continue
        ...
        print("Num of Valid Data:", len(val_data))
    ...
    print("Start convert to range image to size", f"({image_rows}, {image_cols})")
